...
export default function StudentPageClient({ student, essays }) {
  ...
  function openEdit() {
    setEditForm({ name: student.name, email: student.email || '', notes: student.notes || '', level: student.level })
    setEditing(true)
  }
  ...
  return (
    <div>
      <div className="mb-1 text-sm text-muted">
        <Link href="/students" className="hover:text-primary transition-colors">Students</Link> / {student.name}
      </div>

      <PageHeader
        title={student.name}
        subtitle={`${LEVELS[student.level]?.label ?? student.level?.toUpperCase()} · Added ${formatDate(student.created_at)}`}
        action={
          <button
            className="px-4 py-2 rounded-lg border border-border text-sm font-medium text-ink hover:bg-raised transition-colors"
            onClick={openEdit}
          >
            Edit
          </button>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          ...
          { label: 'Level', value: LEVELS[student.level]?.label ?? student.level?.toUpperCase() },
          ...
          <div className="relative bg-surface rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div>
                ...
                <input type="text" className={inputCls} required value={editForm.name || ''} onChange={e => setEditForm({ ...editForm, name: e.target.value })} />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" className={inputCls} value={editForm.notes || ''} onChange={e => setEditForm({ ...editForm, notes: e.target.value })} />
              </div>
              <div>
                <label className={labelCls}>Level</label>
                <select className={inputCls} value={editForm.level || 'c2'} onChange={e => setEditForm({ ...editForm, level: e.target.value })}>
                  {LEVEL_OPTIONS.map(({ value, label }) => <option key={value} value={value}>{label}</option>)}
                </select>
              </div>
              <div className="flex gap-2 justify-end pt-2">
                <button type="button" className="px-4 py-2 rounded-lg text-sm font-medium text-muted hover:bg-raised transition-colors" onClick={() => setEditing(false)}>
                  Cancel
                </button>
